...
export class SignalEngine {
   ...
   */
  private resolveTargetUniverse(symbol: string, category?: string): { assetCategory: 'CRYPTO' | 'FOREX' | 'STOCKS'; universe: string[] } {
  ...
  /**
   * Fast Stage 1 preliminary setup score calculation on 1H candle history.
   * Conserves API limits by screening out flat/weak assets before requesting multi-timeframe data.
   */
  private computeFirstPassScore(symbol: string, htf1h: NormalizedCandle[]): { preliminaryScore: number; direction: SignalDirection; reason: string } {
    if (!htf1h || htf1h.length < 20) {
      return { preliminaryScore: 0, direction: 'BUY', reason: 'Insufficient 1H candle history' };
    }
    ...
    if (!latestClose || latestClose <= 0) {
      return { preliminaryScore: 0, direction: 'BUY', reason: 'Invalid candle close price' };
    }
    ...
    if (!isBullishTrend && !isBearishTrend) {
      return { preliminaryScore: 25, direction: 'BUY', reason: 'Flat/choppy 1H trend' };
    }
    ...
    // Calculate recent 10-bar volatility range
    ...
    const healthyVolatility = rangePct >= 0.15;

    let preliminaryScore = 40;
    ...
    if (emaDiffPct > 0.1) preliminaryScore += 15;

    return {
      preliminaryScore: Math.min(100, preliminaryScore),
      direction,
      reason: `1H Trend: ${direction}, MomentumAligned: ${momentumAligned}, RangePct: ${rangePct.toFixed(2)}%`,
    };
  }

  /**
   * Generates a validated trading signal using a two-stage multi-asset universe scanner.
   * Stage 1: Cheap first-pass screening using cached/1H data.
   * Stage 2: Deep multi-timeframe analysis for top candidates.
   */
  async generateSignal(symbol = 'EURUSD', category?: string): Promise<SignalGenerationResponse> {
    ...
    logger.info(`================================================================`);
    logger.info(`[Multi-Asset Scanner] Initiating Two-Stage Scan for [${assetCategory}] universe across ${universe.length} symbols...`);
    logger.info(`================================================================`);

    // 2. Session Pre-Screening: Filter open assets in universe
    ...
    try {
      // Stage 1: Cheap First-Pass Screening using cached 1H candles
      const stage1Candidates: Array<{
        asset: string;
      ...
      for (const asset of openAssets) {
        ...
        try {
          ...
          logger.info(`[Stage 1 Screen] Skipped ${asset}: 1H candles unavailable (${err instanceof Error ? err.message : String(err)})`);
          continue;
        }

        if (!htf1h || htf1h.length < 20) {
          logger.info(`[Stage 1 Screen] Skipped ${asset}: Insufficient 1H candle history (${htf1h?.length || 0})`);
          continue;
        }
      ...
      if (stage1Candidates.length === 0) {
        logger.info(`[Stage 1 Screen] No assets in ${assetCategory} universe passed preliminary quality screening.`);
        return {
          success: false,
          message: 'NO VALID TRADE OPPORTUNITY',
          symbol: cleanSymbol,
          reason: `Multi-asset Stage 1 screening completed across ${assetCategory} universe (${universe.join(', ')}): No symbols demonstrated sufficient preliminary trend alignment or volatility structure.`,
          timestamp: now,
        };
      }

      // Rank Stage 1 candidates descending by preliminary score
      stage1Candidates.sort((a, b) => b.preliminaryScore - a.preliminaryScore);
      const topCandidates = stage1Candidates.slice(0, 3);
      logger.info(`[Stage 1 Screen] Advancing top ${topCandidates.length} assets to Stage 2 Deep MTF Analysis: ${topCandidates.map(c => `${c.asset} (${c.preliminaryScore}pt)`).join(', ')}`);

      // Stage 2: Deep Multi-Timeframe Analysis & Gate 8 Pipeline
      ...
      for (const cand of topCandidates) {
        ...
        const intervals = ['5m', '15m', '30m', '4h', '1d'];
        for (const interval of intervals) {
          try {
            ...
            logger.debug(`Timeframe '${interval}' unavailable for ${asset}`, { reason: String(err) });
          }
        }
        ...
        if (!validation.isValid) {
          logger.warn(`[Stage 2 Validation Rejected] ${asset}: [${validation.validationReason}] ${validation.detailedMessage}`);
          continue;
        }
      ...
      // Gate 9 Opportunity Ranking & Trade Selection Engine
      const ranking = TradeRankingEngine.rankOpportunities(candidates);
      const validatedSignals = ranking.allRanked;

      if (validatedSignals.length > 0) {
        // Store all ranked validated signals in activeSignals map
        ...
        return {
          success: true,
          message: `Multi-Asset Scan (${assetCategory}): Ranked ${validatedSignals.length} Setup(s). BEST TRADE: ${bestTrade?.symbol || 'N/A'}${secondBest ? ', SECOND BEST: ' + secondBest.symbol : ''}.`,
      ...
      return {
        ...
        symbol: cleanSymbol,
        reason: `Multi-asset scan completed across ${assetCategory} universe (${universe.join(', ')}): No setups satisfied all strict Gate 7–9 quality, ATR volatility, risk-reward ratios, or live price validation hurdles. The system will never force a trade.`,
        timestamp: now,
      };
